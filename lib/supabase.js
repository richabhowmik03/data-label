  ...
  async updateRule(id, updates) {
    if (isSupabaseConfigured) {
      try {
        ...
        // Fallback to in-memory
        const ruleIndex = fallbackRules.findIndex(r => r.id === id);
        if (ruleIndex !== -1) {
          fallbackRules[ruleIndex] = { ...fallbackRules[ruleIndex], ...updates, updated_at: new Date().toISOString() };
          return fallbackRules[ruleIndex];
        }
  ...
  async deleteRule(id) {
    if (isSupabaseConfigured) {
      try {
        ...
        const ruleIndex = fallbackRules.findIndex(r => r.id === id);
        if (ruleIndex !== -1) {
          fallbackRules.splice(ruleIndex, 1);
        }
        return;
      }
    }
  ...
  async processData(payload, labels) {
    ...
    if (isSupabaseConfigured) {
      try {
        ...
        // Fallback to in-memory
        fallbackProcessedData.push(processedEntry);
    ...
    console.log('[DB] Total fallback entries:', fallbackProcessedData.length);
    return processedEntry;
  },
  ...
  async getProcessedData(filters = {}) {
    ...
    if (isSupabaseConfigured) {
      try {
        ...
        console.log('[DB] Falling back to in-memory data:', fallbackProcessedData.length, 'entries');
        return fallbackProcessedData;
      }
    }
    
    console.log('[DB] Using fallback data:', fallbackProcessedData.length, 'entries');
    return fallbackProcessedData;
  }
};
